...
function hookPointsToWays(vgiStop, ways, snapTolerance) {
  function snapPointOnLineString(way, vgiStop, snapTolerance) {
    ...
    if(!props.osm) throw Error(`error with way in hookPointsToWays`);
  ...
  const hookPoints = ways.map(w => snapPointOnLineString(w, vgiStop, snapTolerance));
...
function snakeWaysLength(ways) {
  let waykm = 0;
  ways.forEach(sw => waykm += lineLength(sw.geometry.coordinates));
  return (waykm / 1000).toFixed(2);
}


// inner function of the loop over the corridors
function addOneCorridor(Aend, Bend, corridorIndex, coarseWays, snakeWays, corridorBuffer, snapTolerance) {
  // 1. restrict coarseWays to the A-to-B-corridor
  let okWays = restrictToCorridor(Aend, Bend, coarseWays, corridorIndex, corridorBuffer, snakeWays);
  const snake0 = snakeWays.at(-1);
  if(snake0 && notInSnake(snake0, okWays)) okWays = okWays.concat(snakeWays); // WARNING : check this FIRST TODO
  console.log(`°°°°°°°°°° \t\t CORRIDOR ways: ${okWays.map(w => w.properties.osm).join()}`);
  // 2. check again Asnap against okWays
  const Ahookpoints = hookPointsToWays(Aend, okWays, snapTolerance);
  console.log(`Ahookpoints ${corridorIndex} = ${Ahookpoints.length} ${Ahookpoints.map(w => JSON.stringify(w.properties)).join()}`);
  const Bhookpoints = hookPointsToWays(Bend, okWays, snapTolerance);
  let Asnap = Ahookpoints[0], osmInit = Ahookpoints[0].properties.osm;
  let wayInit = corridorIndex ? snakeWays[0] : wayOfFrom(coarseWays, osmInit);
  console.log(`wayInit ${JSON.stringify(wayInit.properties)}`);
  if(Ahookpoints[0].properties.closestEnd) {
    wayInit = bridgePointToClosestWayStart(Asnap, Aend);
    if(wayInit) snakeWays = augmentsSnakeWith(snakeWays, wayInit);
  ...
  let Bsnap = Bhookpoints[0], osmLast = Bhookpoints[0].properties.osm;
  let wayLast = wayOfFrom(coarseWays, osmLast);
  console.log(`===== ===== ===== corridor ${corridorIndex} from ${Aend.properties.id} to ${Bend.properties.id}`);
  console.log(`initial osmInit/osmLast: ${osmInit} ${osmLast}`);

  // 3. removes ways already in the last corridor snakeWays (exception: at turn back stations)
  okWays = okWays.filter(w => !snakeWays.some(sw => sw.properties.osm === w.properties.osm));
  console.log(`current okWays: ${okWays.map(w => w.properties.osm).join()}`);

  // 4. try to concat okWays
  if(okWays.length > 0) {
    let corridorWays = [];
    okWays.forEach(w => {
    ...
    console.log(`current corridorWays: ${corridorWays.map(w => w.properties.osm).join()}`);
  ...
  //  if(Cway) snakeWays = snakeWays.concat(Cway);
    ...
    else console.log(`null Cway at: ${corridorIndex}`);
  }
  if(Bsnap.properties.closestEnd) {
    wayLast = bridgeClosestWayEndToPoint(Bsnap, Bend);
    if(wayLast) snakeWays = augmentsSnakeWith(snakeWays, wayLast);
    Bsnap = hookPointsToWays(Bend, [wayLast], snapTolerance)[0];
  ...
  console.log(`current snake: ${snakeWays.map(w => w.properties.osm).join()}, total length= ${snakeWaysLength(snakeWays)}`);
  snakeWays.Bsnap = Bsnap;
  return snakeWays;
}
//
function loopoverCorridors(stopFeatures, coarseWays, corridorBuffer, snapTolerance) {
  ...
  let Bend = Aend, Bsnap = Asnap, BHook = AHook;
  ...
  for (let i = 0; i < stopFeatures.length - 1; i++) {
    ...
    snakeWays = addOneCorridor(Aend, Bend, i, coarseWays, snakeWays, corridorBuffer, snapTolerance);
    Bsnap = snakeWays.Bsnap;
  }
}
...
function concatWaysInSnake(initialWay, ways, osmLast) {
  ...
  function doMergeWays(currWay, sequelWay, reverseTag) {
    ...
    const sequelOsm = sequelWay.properties.osm;
    console.log(`concat ${reverseTag?"reverse ":""}${group.join()} + ${sequelOsm}`);
    group.push(sequelOsm);
    currWay.properties.group = group;
    currWay.properties.osm = sequelOsm;
    // update coordinates
...
//  const lastCoord = coords[coords.length - 1];
    ...
    for(const w of ways) {
      ...
      if (sameCoord(start, lastCoord)) {
        reverse = false;
        currWay = doMergeWays(currWay, w, reverse);
        nextFeature = w; break;
      }
